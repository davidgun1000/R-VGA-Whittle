run_mcmc_sv <- function(y, #sigma_eta, sigma_xi, 
  ...
  rho <- 0.99 # correlation factor for correlated PMMH
  
  U <- post_samples <- post_samples_theta <- matrix(NA, iters, param_dim)
  ...
  ## Correlate the random numbers here
  u_curr <- rnorm(param_dim, 0, 1)
  theta_curr <- as.vector(prior_mean + t(chol(prior_var)) %*% u_curr) #rmvnorm(1, prior_mean, prior_var)
  ...
  for (i in 1:iters) {
    ...
    v <- rnorm(param_dim, 0, 1)
    u_prop <- rho * u_curr + sqrt(1 - rho^2) * v
    
    theta_prop <- as.vector(theta_curr + t(chol(D)) %*% u_prop)
    U[i, ] <- theta_prop
    ...
    ## 3. Calculate prior
    log_prior_prop <- dmvnorm(theta_prop, t(prior_mean), prior_var, log = T)
