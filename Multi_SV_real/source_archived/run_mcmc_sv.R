run_mcmc_sv <- function(y, #sigma_eta, sigma_xi, 
  ...
  if (use_whittle_likelihood) {
    ...
    log_likelihood_curr <- compute_whittle_likelihood_sv(y = y, params = params_curr)$log_likelihood
  } else {
  ...
  for (i in 1:iters) {
    ...
    if (use_whittle_likelihood) {
      params_prop <- list(phi = phi_prop, sigma_eta = sigma_eta_prop, 
                          sigma_xi = sigma_xi)
      log_likelihood_prop <- compute_whittle_likelihood_sv(y = y, params = params_prop)$log_likelihood
    } else {
