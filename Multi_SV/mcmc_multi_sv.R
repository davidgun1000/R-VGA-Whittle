...
param_grid <- seq(0.1, 2, length.out = 100)
...
for (j in 1:length(param_grid)) {
  cat("j =", j, "\n")
  Sigma_eta_j <- diag(c(param_grid[j]^2, Sigma_eta[2,2]))
  Sigma_eta_j_2 <- diag(c(Sigma_eta[1,1], param_grid[j]^2))
  multi_llh1[j] <- compute_whittle_likelihood_multi_sv(Y, params = list(Phi = Phi,
                                                                 Sigma_eta = Sigma_eta_j))$log_likelihood
  multi_llh2[j] <- compute_whittle_likelihood_multi_sv(Y, params = list(Phi = Phi,
                                                                        Sigma_eta = Sigma_eta_j_2))$log_likelihood

}

par(mfrow = c(2,2))
plot(param_grid, multi_llh1, type = "l")
abline(v = Sigma_eta[1,1], lty = 2)
abline(v = param_grid[which.max(multi_llh1)], lty = 2, col = "red")

plot(param_grid, multi_llh2, type = "l")
abline(v = Sigma_eta[2,2], lty = 2)
abline(v = param_grid[which.max(multi_llh2)], lty = 2, col = "red")

# llh1 <- c()
# llh2 <- c()
# for (j in 1:length(param_grid)) {
#   sigma_eta_j <- param_grid[j]
#   llh1[j] <- compute_whittle_likelihood_sv(y = Y[1, ], params = list(phi = Phi[1,1],
#                                                                  sigma_eta = sigma_eta_j))$log_likelihood
#   llh2[j] <- compute_whittle_likelihood_sv(y = Y[2, ], params = list(phi = Phi[2,2],
#                                                                      sigma_eta = sigma_eta_j))$log_likelihood
# }
# # par(mfrow = c(1,1))
# plot(param_grid, llh1, type = "l")
# abline(v = Sigma_eta[1,1], lty = 2)
# abline(v = param_grid[which.max(llh1)], lty = 2, col = "red")
# 
# plot(param_grid, llh2, type = "l")
# abline(v = Sigma_eta[2,2], lty = 2)
# abline(v = param_grid[which.max(llh2)], lty = 2, col = "red")
# #
# browser()
# 
...
#                              prior_mean = rep(0,2), prior_var = diag(2),
...
# 
# par(mfrow = c(2,1))
# coda::traceplot(mcmcw1.post_samples_phi, main = "Trace plot for phi")
# coda::traceplot(mcmcw1.post_samples_sigma_eta2, main = "Trace plot for sigma_eta2")
# 

## Also test HMC on a single series

# stan.iters <- n_post_samples + burn_in
#
# hfit <- run_hmc_sv(data = Y[2, ], iters = stan.iters, burn_in = burn_in)
#
...
#
# hmc.post_samples_phi <- tanh(hmc.theta_phi)
# hmc.post_samples_sigma_eta2 <- exp(hmc.theta_sigma)
#
#
# par(mfrow = c(1,2))
# plot(density(mcmcw.post_samples_phi), main = "Posterior of phi",
#      col = "blue", lty = 2, lwd = 1.5)
# lines(density(hmc.post_samples_phi), col = "forestgreen")
# abline(v = Phi[1,1], lty = 3)
# #
# plot(density(mcmcw.post_samples_sigma_eta2), main = "Posterior of sigma_eta",
#      col = "blue", lty = 2, lwd = 1.5)
# lines(density(hmc.post_samples_sigma_eta2), col = "forestgreen")
# abline(v = Sigma_eta[2,2], lty = 3)
# #
# browser()
#############################
##   MCMC implementation   ##
...
par(mfrow = c(3,2))
coda::traceplot(mcmcw.post_samples_phi_11, main = "Trace plot for MCMC with exact likelihood")
abline(h = phi11, col = "red", lty = 2)
coda::traceplot(mcmcw.post_samples_phi_12, main = "Trace plot for MCMC with exact likelihood")
abline(h = phi12, col = "red", lty = 2)
coda::traceplot(mcmcw.post_samples_phi_21, main = "Trace plot for MCMC with exact likelihood")
abline(h = phi21, col = "red", lty = 2)
coda::traceplot(mcmcw.post_samples_phi_22, main = "Trace plot for MCMC with exact likelihood")
abline(h = phi22, col = "red", lty = 2)

coda::traceplot(mcmcw.post_samples_sigma_eta_11, main = "Trace plot for MCMC with exact likelihood")
...
coda::traceplot(mcmcw.post_samples_sigma_eta_22, main = "Trace plot for MCMC with exact likelihood")
