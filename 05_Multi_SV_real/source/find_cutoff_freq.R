...
find_cutoff_freq <- function(signal, nsegs, power_prop = 0.5) {
    ...
    pdg_out <- compute_periodogram_uni(y_tilde)
    ...
    cutoff_ind <- which(pdg_welch < half_power)[1]  # the 3dB cutoff is immediately before the first frequency that dips below half power
