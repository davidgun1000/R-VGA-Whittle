construct_prior <- function(data, prior_type = "prior1", 
  ...
  if (use_cholesky) {
    param_dim <- m + (m*(m-1)/2 + m) # m AR parameters, 
...
#     prior_mean <- rep(0, param_dim)
#   } else {
#     # prior_mean <- c(rep(0, m), rep(-1, m), rep(0, m*(m-1)/2))
#     prior_mean <- c(rep(0, m), rep(-2, m), rep(0, m*(m-1)/2))
