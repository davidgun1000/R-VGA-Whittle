run_rvgaw_multi_sv <- function(data, prior_mean, prior_var, S,
  ...
  # I_tf <- tf$Variable(I[,,j])
  # freq_tf <- tf$Variable(freq[j])
  # 
  # llh_tf <- compute_grad_hessian(samples_tf, I_i = I_tf, freq_i = freq_tf)
  # 
  # llh <- list()
  # 
  # for (s in 1:S) {
  ...
  #   llh[[s]] <- compute_partial_whittle_likelihood(Y = Y,
  #                                                params = list(Phi = Phi_s,
  ...
  for (i in 1:length(freq)) {
    ...
    for (v in 1:length(a_vals)) { # for each step in the tempering schedule
      ...
      # Sigma_eta_samples_tf <- tf$linalg$diag(samples_tf[, 5:6])
      
      # llh_test <- c()
      # for (s in 1:nrow(samples)) {
      #   Sigma_eta_s <- matrix(diag(exp(samples[s, 5:6])), 2, 2, byrow = T)
      #   A_s <- matrix(samples[s, 1:4], 2, 2, byrow = T)
      #   
      #   Phi_s <- backward_map(A_s, Sigma_eta_s)
      #   
      #   llh_test[s] <- compute_partial_whittle_likelihood(Y, params = list(Phi = Phi_s,
      #                                                                      Sigma_eta = Sigma_eta_s),
      #                                                     j = i)
      # }
