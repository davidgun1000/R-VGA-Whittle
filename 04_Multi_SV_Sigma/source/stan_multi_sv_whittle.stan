functions {
  ...
  real complex_wishart_lpdf (array[] complex_matrix W, int v, array[] complex_matrix Sigma) {
    ...
    int nfreq = size(W);
    complex det_Sigma;
    complex_matrix[rows(Sigma[2]), rows(Sigma[1])] inv_Sigma;
    ...
    for (k in 1:nfreq) {
      ...
      det_Sigma = Sigma[k][1,1] * Sigma[k][2,2] - Sigma[k][2,1] * Sigma[k][1,2];
      inv_Sigma[1,1] = Sigma[k][2,2];
      inv_Sigma[2,2] = Sigma[k][1,1];
      inv_Sigma[2,1] = -Sigma[k][2,1];
      inv_Sigma[1,2] = -Sigma[k][1,2];
      inv_Sigma = 1/det_Sigma * inv_Sigma;
      
      log_likelihood_k = -(sum_log_eig_Sigma + trace(W[k] * inv_Sigma));
...
model {
  ...
  periodogram ~ complex_wishart(1, spec_dens);
  // target += complex_wishart_lpdf(periodogram | 1, spec_dens);
}
