...
if (rerun_vb) {
  ...
  mu_0 <- c(0, log(sigma_e^2))
  sigma_0 <- diag(c(1, 0.5))
  ...
  for (k in 2:VB_iters) {
    ...
    tau <- rnorm(length(lambda$mu), 0, 1)
    ...
    if (use_whittle_likelihood) {
      ...
      grad_theta_phi <- (2*cos(freq)*(tanh(theta_phi)^2 - 1) -
                           2*tanh(theta_phi)*(tanh(theta_phi)^2 - 1)) /
        (tanh(theta_phi)^2 - 2*cos(freq)*tanh(theta_phi) + 1) -
        I*exp(-theta_sigma)*(2*cos(freq)*(tanh(theta_phi)^2 - 1) -
                                      2*tanh(theta_phi)*(tanh(theta_phi)^2 - 1))
      
      grad_theta_sigma <- I*exp(-theta_sigma)*(tanh(theta_phi)^2 - 2*cos(freq)*tanh(theta_phi) + 1) - 1
      
      grad_llh <- c(grad_theta_phi, grad_theta_sigma) 
    ...
    grad_log_prior_theta_phi <- - 1/prior_var_theta_phi * (theta_phi - prior_mean_theta_phi)#- theta_phi
    grad_log_prior_theta_sigma <- - 1/prior_var_theta_sigma * (theta_sigma - prior_mean_theta_sigma)
    grad_log_prior <- c(grad_log_prior_theta_phi, grad_log_prior_theta_sigma)
