...
method_names <- rep(c("rvgaw", "rvgae", "vbw", "vbe", "mcmcw", "mcmce"), each = n_post_samples)
likelihood_type <- rep(c(rep("Whittle", n_post_samples), rep("Exact", n_post_samples)), times = 3)
...
g <- ggplot(all_df, aes(x = post_samples, colour = method, linetype = likelihood)) +
            ...
            scale_linetype_manual(name = "method", values=rep(c(1, 2), 3)) +
            theme_bw() + 
...
## Trajectories
plot_range <- 1:n #400:1000#floor(n/2)
if (reorder_freq) {
...
plot(tanh(unlist(rvgae_results$mu)[plot_range]), type = "l",
